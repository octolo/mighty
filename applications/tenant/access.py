...
class TenantAccess:
    ...
    def tenant_roles_pk(self):
        return self.role_model.objects.filter(roles_tenant__user=self.request.user).values_list("pk", flat=True)
    ...
    def tenant_groups_pk(self):
        return self.request.user.user_tenant.values_list("group_id", flat=True)
