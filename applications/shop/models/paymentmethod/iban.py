...
class IbanModel:
    ...
    def readable_iban(self):
        return ' '.join([
            self.iban[i : i + 4] for i in range(0, len(self.iban), 4)
        ])
    ...
    def mask_iban(self):
        iban = (
            ...
            + re.sub(r'[a-zA-Z0-9]', '*', self.iban_readable[4:-4])
            + self.iban_readable[-4:]
        )
    ...
    def is_exist_iban(self):
        if ShopConfig.subscription_for == 'group':
            qs = type(self).objects.filter(
                iban=self.iban, bic=self.bic, group=self.group
            )
        else:
            qs = type(self).objects.filter(
                iban=self.iban, bic=self.bic, user=self.user
            )
        if self.pk:
    ...
    def is_valid_iban(self):
        ...
        if not self.is_exist_iban:
            raise ValidationError(
                code='already_iban', message='IBAN already exist'
            )
