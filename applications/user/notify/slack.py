...
class SlackUser(NotifyBackend):
    ...
    def slack_self(self):
        ...
        if self.user.email or self.user.phone:
            base += (
                ' - ' + self.user.email if self.user.email else self.user.phone
            )
        return base
        ...
        return [
            {
                ...
                'text': {
                    'type': 'plain_text',
                    'text': ':baby: New user on the platform : {}'.format(
                        self.user.date_create.strftime('%Y-%m-%d %H:%M')
                    ),
                    'emoji': True,
                },
            },
    ...
    def send_msg_create(self):
        text = 'New user on the platform : {}'.format(
            self.user.date_create.strftime('%Y-%m-%d %H:%M')
        )
