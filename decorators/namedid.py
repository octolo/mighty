        ...
        class NewClass(obj):
            ...
            def Qasfield(self):
                return {
                    ...
                    for field in self.Qnamedfields
            ...
            def set_named_id(self, offset=0):
                base_name = '-'.join([
                    str(getattr_recursive(self, field))
                    for field in self.named_id_fields
                    if getattr_recursive(self, field)
                ])
                named_id = slugify(base_name)
                count = self.count_named_id + offset
                self.named_id = (
                    f'{named_id}-{count + 1}' if count > 0 else named_id
                )
                if self.named_id_exist:
