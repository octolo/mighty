class FormDescriptor:
    ...
    validators = {'regexvalidator': ('code', 'message', 'pattern')}
    ...
    def input_type_field(self, field):
        ...
        if hasattr(field.widget, 'input_type'):
            return self.formats_input.get(
                field.widget.input_type, field.widget.input_type
            )
        return self.formats_input.get(
    ...
    def config_field(self, field):
        ...
        if hasattr(field, 'choices'):
            config['options'] = self.choices_field(field)
        if hasattr(field, 'choices_enhanced'):
            config['options_enhanced'] = self.choices_field(field)
        if hasattr(field, 'min_length'):
            config['min_length'] = field.min_length
        if hasattr(field, 'max_length'):
            config['max_length'] = field.max_length
        if hasattr(field, 'vars_url'):
            config['vars_url'] = field.vars_url
    ...
    def many_field(self, field):
        return field.many if hasattr(field, 'many') else False

    def isObj_field(self, field):
        return field.isObj if hasattr(field, 'isObj') else None

    def isArray_field(self, field):
        return field.isArray if hasattr(field, 'isArray') else None

    def errors_field(self, field):
        return field.error_messages

    def attrs_field(self, field):
        return getattr(field.widget, 'attrs', {})
    ...
    def hasIcon_field(self, field):
        return (
            field.icon
            if hasattr(field, 'icon')
            else self.defaultIcon_field(field)
        )
    ...
    def config_validator(self, field, validator):
        ...
        if name in self.validators:
            config = {'name': name}
            config.update({
                val: self.get_config_validator(val, field, validator)
    ...
    def validators_fields(self, field):
        if len(field.validators):
            validators = [
                self.config_validator(field, validator)
    ...
    def dependencies_field(self, name):
        return (
            getattr(self.form, f'{name}_dependencies')
            if hasattr(self.form, f'{name}_dependencies')
            else None
        )

    def choice_dependencies_field(self, name):
        return (
            getattr(self.form, f'{name}_choice_dependencies')
            if hasattr(self.form, f'{name}_choice_dependencies')
            else None
        )

    def emptyif_field(self, name):
        return (
            getattr(self.form, f'{name}_emptyif')
            if hasattr(self.form, f'{name}_emptyif')
            else None
        )

    def get_fields(self):
        return [
            self.field_definition(field, name)
