...
if(!RGraph.SVG.REG.get('tooltip')){var tooltipObj=document.createElement('DIV');tooltipObj.className=prop.tooltipsCssClass;for(var i in RGraph.SVG.tooltips.style){if(typeof i==='string'){tooltipObj.style[i]=substitute(RGraph.SVG.tooltips.style[i]);}}
...
var tooltipsFormattedKeyColorsCss='';if(prop.tooltipsFormattedKeyColorsCss){for(property in prop.tooltipsFormattedKeyColorsCss){if(typeof property==='string'){tooltipsFormattedKeyColorsCss+='{1}: {2};'.format(property.replace(/[A-Z]/,function(match)
{return'-'+match.toLowerCase();}),String(prop.tooltipsFormattedKeyColorsCss[property]));}}}
...
{var str='<table>';if(prop.tooltipsFormattedTableHeaders&&prop.tooltipsFormattedTableHeaders.length){str+='<thead><tr>';for(var i=0;i<prop.tooltipsFormattedTableHeaders.length;++i){str+='<th>'+prop.tooltipsFormattedTableHeaders[i]+'</th>';}
str+='</tr></thead>';}
if(typeof prop.tooltipsFormattedTableData==='object'&&!RGraph.isNull(prop.tooltipsFormattedTableData)){str+='<tbody>';for(var i=0;i<prop.tooltipsFormattedTableData[specific.sequentialIndex].length;++i){str+='<tr>';for(var j=0;j<prop.tooltipsFormattedTableData[specific.sequentialIndex][i].length;++j){str+='<td>'+String(prop.tooltipsFormattedTableData[specific.sequentialIndex][i][j])+'</td>';}
str+='</tr>';}
str+='</tbody>';}
