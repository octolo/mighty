var MightyCore = function(options) {
    ...
    this.translation = {
        ...
        displayall: this._icons.displayall + " Show all",
        displaynone: this._icons.displaynone + " Hide all",
    };
    ...
    this.last = function(config, what, like) {
        ...
        if (this._last[config].hasOwnProperty(what) && this._last[config][what] === like) {
            return true;
        }
        this._last[config][what] = like;
        return false
    }
    ...
    this.del = function(config, key) {
        this.log("debug", "del", [config, key]);
        if (!this.configs.hasOwnProperty(config) && this.configs[config].hasOwnProperty(key)) {
            delete this.configs[config][key]; 
        }
    }
