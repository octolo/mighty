...
class Filter(Verify):
    ...
    def get_Q(self, exclude=False):
        value = self.format_value(exclude)
        return Q(**{self.get_field(value): value})
...
class ParamMultiChoicesFilter(ParamChoicesFilter):
    def __init__(self, id, request=None, *args, **kwargs):
        super().__init__(id, request, *args, **kwargs)
    ...
    def get_value(self, exclude=False):
        return "W"
        #if self.choices_required:
        #    return [value for value in super()from django.db.models import Q
from mighty import Verify
from mighty.functions import make_searchable, test, get_logger, make_float
from mighty.apps import MightyConfig
...
class FilterByGTEorLTE(ParamMultiChoicesFilter):
    ...
    def get_Q(self, exclude=False):
        return reduce(self.operator, [Q(**{self.get_field(value): self.format_value(value) }) for value in self.get_value(exclude)])
