...
class ModelViewSet(ModelViewSet):
    ...
    def reporting(self, request, *args, **kwargs):
        ...
        if (
            any(report, filetype)
            and filetype in self.file_type
            and report in obj.reporting_keys
        ):
            return obj.reporting_process(reporting, file_type)
        raise Http404
    ...
    def form_desc(self, request, form=None, *args, **kwargs):
        ...
        if desc:
            formdesc = FormDescriptor(
                desc, self.request, drf_kwargs=self.kwargs, **kwargs
            ).as_json()
    ...
    @action(
        ...
        url_path=r'saction/(?P<saction>\w+)',
    )
    ...
    @action(
        detail=True,
        methods=['get', 'post', 'delete'],
        url_path=r'daction/(?P<daction>\w+)',
    )
    def data_action(self, *args, **kwargs):
        ...
        if action in self.actions_authorized:
            ...
            if hasattr(obj, daction):
                result = getattr(obj, daction)(
                    self.request.data, self.request.method
                )
                return Response(result)
        raise Http404
    ...
    def table_desc(self, request, table=None, *args, **kwargs):
        desc = next(
            (f for t in self.tables_desc if t.Options.url == table), None
        )
        if desc:
    ...
    def variables(self, request, *args, **kwargs):
        if hasattr(self.model_static, 'eve_variable_prefixed_list'):
            var = self.model_static().eve_variable_prefixed_list(
                related=self.variables_related
            )
    ...
    def foxid(self):
        return Foxid(
            ...
            order_enable=self.order_enable,
            order_association=self.order_association,
        ).ready()

    def foxid_qs(self, qs, flts, order_base=None):
        return Foxid(
            qs, self.request, f=flts, order_base=order_base or self.order_base
        ).ready()
