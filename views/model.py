    ...
    def get_context_data(self, **kwargs):
        ...
        context.update({
            ...
            'can_change': self.request.user.has_perm(
                context['fake'].perm('change')
            ),
            'can_delete': self.request.user.has_perm(
                context['fake'].perm('delete')
            ),
            'can_export': self.request.user.has_perm(
                context['fake'].perm('export')
            ),
            'can_import': self.request.user.has_perm(
                context['fake'].perm('import')
            ),
            'can_disable': self.request.user.has_perm(
                context['fake'].perm('disable')
            ),
            'can_enable': self.request.user.has_perm(
                context['fake'].perm('enable')
            ),
        })
        return context
